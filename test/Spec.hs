...
fromYamlTests = testGroup "Document from yaml"
  [   testCase "null" $
    ...
    , testCase "Nested DMap2 style" $
        parseDocument cmp23 @?= Right (DMap [("coords",DMap [("head",DMap [("col",DInteger 1),("row",DInteger 2)]),("tail",DMap [("head",DMap [("col",DInteger 1)]),("tail",DNull)])])])
